...
import {list_customer_inbox} from '../../actions/inbox'
...
const Screen  = ({auth,list_customer_inbox}) => {
    ...
    useEffect(async () => {
        ...
        const res = await list_customer_inbox()
        if(res.success){
                        ...
                        <TableBody>
                            {inboxs.map((data) =>{
                                ...
                                <TableCell component="th" scope="row">
                                    <Typography style={{fontSize:18,fontFamily:'kanit-regular',paddingLeft:20}} >แอดมินตงสึ่น</Typography>   
                                </TableCell>
                                <TableCell component="th" scope="row">
                                ...
                                <TableCell align="right">
                                    ...
                                    <Typography style={{fontSize:18,fontFamily:'kanit-light',paddingRight:20}} >{moment(data.create_date).format('D/M/yyyy')}</Typography>   </TableCell>
                                </TableRow>
                            })}
                    ...
                    <Stack spacing={2} style={{marginTop:20}}>
                        {total/limit > 1 &&
                            <Pagination count={Math.ceil(total/limit)} page={page} size="small" onChange = {async (event, value)=>{
                                    ...
                                    const res3 = await list_customer_inbox(query)
                                    if(res3.success){
...
export default connect(mapStateToProps,{list_customer_inbox})(Screen)
